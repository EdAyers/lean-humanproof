...
universes u v w
def list.mchoose {m : Type u → Type v} [monad m] {α : Type w} {β : Type u} 
    (f : α → m (option β)) : list α → m (list β)
| []       := return []
| (h :: t) := pure (λ (h : option β) (t : list β), option.rec_on h t (λ h, h :: t)) <*> f h <*> list.mchoose t 

def list.concat {α : Type u} : list α -> list α -> list α
| [] b := b
| (h :: t) b := h :: (list.concat t b) 
...
meta structure formula :=
    (term : expr)(type : expr)(deps : exprset)
...
do { 
    ...
    --let dangling := find_dangling target_deps [] hyps in
